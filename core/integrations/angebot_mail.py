...
def _build_rechnung_mail_html(
    *,
    ...
    """Apple-clean Mail-Body fuer Rechnungs-Versand mit PDF-Anhang."""
    anrede = f"Hallo {_html.escape(kunde_anrede)}," if kunde_anrede else "Hallo,"
    summe = _format_eur(gesamtbetrag_brutto_eur) if gesamtbetrag_brutto_eur else ""
    summe_block = (
        f'<p style="margin: 0 0 16px;">Rechnungsbetrag: <b>{summe}</b> brutto. '
        f'Zahlbar innerhalb von 14 Tagen.</p>'
        if summe else ""
    )
    contact_lines = []
    if contact_name:
...
<p style="margin:0 0 16px;">vielen Dank fuer den Auftrag. Anbei finden Sie unsere Rechnung
{f"<b>{_html.escape(rechnung_nummer)}</b>" if rechnung_nummer else ""} als PDF im Anhang.</p>
{summe_block}
<p style="margin:0 0 16px;">Bei Rueckfragen antworten Sie einfach auf diese Mail.</p>
<p style="margin:24px 0 0;">Viele Gruesse<br>
{contact_html or _html.escape(company_name)}</p>
</td></tr>
</table>
...
async def send_rechnung_to_customer(
    *,
    angebot_id: UUID,
    to_email: str,
    cc: Optional[list[str]] = None,
) -> dict:
    """Versendet die Lexware-Rechnung (PDF) an den Kunden — analog zu
    send_angebot_to_customer aber fuer den Rechnungs-Lifecycle.

    Voraussetzung: ang.lexware_invoice_id ist gesetzt UND die Rechnung
    in Lexware ist nicht mehr im Draft-Status (sonst kein PDF-Download).
    """
    ...
    async with AsyncSessionLocal() as session:
        ...
        if ang is None:
            out["error"] = "Auftrag nicht gefunden"
            return out
        tenant = (await session.execute(
            select(Tenant).where(Tenant.id == ang.tenant_id)
        )).scalar_one_or_none()
    if tenant is None:
        out["error"] = "Tenant nicht gefunden"
        return out
    if ang.lexware_invoice_id is None:
        out["error"] = "Keine Lexware-Invoice-ID am Auftrag — erst /auftrag finalisieren"
        return out

    # 2) Lexware-Provider aus Tenant-Config
    provider = None
    async with AsyncSessionLocal() as session:
        tc = (await session.execute(
            select(ToolConfig).where(
    ...
    # 3) Invoice-Status checken — muss finalisiert sein fuer PDF-Download
    try:
        inv = await provider.get_invoice(ang.lexware_invoice_id)
    except AccountingError as e:
        out["error"] = f"Lexware get_invoice: {e}"
        return out

    voucher_status = (inv.get("voucherStatus") or "").lower()
    if "draft" in voucher_status:
        out["error"] = (
            "Rechnung ist in Lexware noch im Draft-Status. "
            "Bitte oeffnen und 'Ausstellen' druecken, dann erneut versenden."
        )
        return out

    # 4) PDF holen
    try:
        pdf_bytes = await provider.download_invoice_pdf(ang.lexware_invoice_id)
    except AccountingError as e:
        out["error"] = f"PDF-Download: {e}"
        return out
    except Exception as e:
    ...
    # 5) Mail bauen
    nummer = inv.get("voucherNumber") or ""
    kunde_anrede = (ang.kunde_name or "").split()[-1] if ang.kunde_name else ""
    body_html = _build_rechnung_mail_html(
        kunde_anrede=kunde_anrede,
        rechnung_nummer=str(nummer),
        gesamtbetrag_brutto_eur=ang.gesamtbetrag_brutto_eur,
        company_name=tenant.company_name or "Handwerksbetrieb",
        contact_name=getattr(tenant, "contact_name", "") or "",
        contact_email=getattr(tenant, "contact_email", "") or "",
    ...
    subject_nummer = f" {nummer}" if nummer else ""
    subject = f"Ihre Rechnung{subject_nummer} von {tenant.company_name or 'uns'}"
    filename = f"Rechnung{('-' + str(nummer)) if nummer else ''}.pdf"
    ...
    # die Rechnungs-Mail mit, separates Tracking ist Phase 2.
    async with AsyncSessionLocal() as session:
        ang_db = (await session.execute(
            select(Angebot).where(Angebot.id == angebot_id)
        )).scalar_one_or_none()
    ...
    out["message_id"] = result.get("message_id")
    logger.info(
        f"send_rechnung_to_customer OK: angebot={angebot_id} to={to_email}"
    )
    return out
