...
und `create_owner_activation` erzeugt den **sicheren** Inhaber-
Aktivierungs-Token.

Sicherheit (S13): Der Inhaber wird ueber einen einmaligen, zufaelligen
Aktivierungs-Token gebunden — exakt derselbe Mechanismus wie beim
Mitarbeiter-Onboarding. Der Token steckt im Link
`/app/activate?token=…`, der dem Kunden per Mail zugeschickt wird.
"""
...
# dem Katalog, ausgenommen always-on (kunde_lookup) und
...
async def create_tenant_record(
    *,
    ...
    async with AsyncSessionLocal() as session:
        ...
        # bindet den App-Zugang an genau diesen Employee.
...
async def create_owner_activation(
    ...
    *,
    ...
    """Erzeugt einen einmaligen Aktivierungs-Token fuer das Onboarding.

    Gibt die Token-Zeile zurueck — `.token` gehoert in den Link
    `/app/activate?token=…`, mit dem der Inhaber sein Passwort setzt.
    """
