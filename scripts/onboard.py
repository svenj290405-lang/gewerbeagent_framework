...
async def onboard_tenant(
    ...
    print("     c) Voice-Tools auf voice_init zeigen lassen (NICHT direkt auf")
    print("        kalender — voice_init ruft den Kalender intern auf):")
    print(f"        Basis: {settings.public_url}/webhook/{slug}/voice_init/<tool>")
    print("        WICHTIG: an JEDEM Voice-Tool den Custom-Header setzen:")
    print("            X-Webhook-Secret: <ELEVENLABS_WEBHOOK_SECRET aus .env>")
    print("        Ohne Header -> 401 (fail-closed). Der direkte kalender-")
    print("        Webhook ist jetzt komplett gesperrt (internal-only, 404).")
